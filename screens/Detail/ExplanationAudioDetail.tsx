...
export default function ExplanationAudioDetail() {
  ...
  const fetchExplanationAudioDetail = async () => {
    const response = await api.get(
      `library/explanationAudioAPI/products/${explanationAudioId}/`,
    );
    return response;
  };

  const {data, isLoading, isError, error, isRefetching, refetch} = useQuery({
    queryKey: ['explanation audio', 'detail', `${explanationAudioId}`],
    queryFn: fetchExplanationAudioDetail,
  });
      ...
      <View style={styles.container}>
        ...
        <ScrollView
          ...
          <View style={styles.explanationAudioDetailContainer}>
            ...
            {data?.data.is_bought ? null : (
              <>
                ...
                <View
                  ...
                  {!cart.find(item => item.id === explanationAudioId) ? (
                    <>
                      <TouchableOpacity
                    ...
                    <>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => removeItem(explanationAudioId)}>
            ...
            <Text>
              <Text style={{fontWeight: '900', fontSize: 16}}>Chapters:</Text>{' '}
              {data?.data.chapters.length}
            </Text>
            <Text>
              <Text style={{fontWeight: '900', fontSize: 16, width: '100%'}}>
                Description:
              </Text>{' '}
...
const styles = StyleSheet.create({
  ...
  explanationAudioDetailContainer: {
    padding: 20,
    display: 'flex',
    justifyContent: 'center',
    gap: 10,
